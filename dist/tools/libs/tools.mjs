import { n as __toESM } from "../../rolldown-runtime.mjs";
import { t as require_semver } from "./semver.mjs";
...
var ArgumentsBuilder = class {
	...
	*/
	addArgument(value) {
		if (value) this.args.push(this.escapeArgument(value));
		return this;
	}
	/**
	...
	*/
	addArguments(values) {
		for (const value of values) this.addArgument(value);
		return this;
	}
	/**
	...
	*/
	addKeyValueEquals(key, value) {
		if (key && value !== void 0 && value !== null) this.args.push(`--${key}=${this.escapeArgument(value)}`);
		return this;
	}
	/**
	...
	*/
	escapeArgument(value) {
		...
		if (this.isWindows) return `"${value.replace(/\\/g, "\\\\").replace(/"/g, "\\\"")}"`;
	...
	*/
	needsEscaping(value) {
		...
		if (this.isWindows) return windowsNeedsEscaping.test(value);
...
var DotnetTool = class DotnetTool {
	...
	async install() {
		...
		let version = import_semver.clean(setupSettings.versionSpec) || setupSettings.versionSpec;
		...
		if (!this.isExplicitVersion(version)) {
			...
			if (!version) throw new Error(`Unable to find ${this.packageName} version '${version}'.`);
		}
		if (this.versionRange && !import_semver.satisfies(version, this.versionRange, { includePrerelease: setupSettings.includePrerelease })) throw new Error(`Version spec '${setupSettings.versionSpec}' resolved as '${version}' does not satisfy the range '${this.versionRange}'.See https://github.com/GitTools/actions/blob/main/docs/versions.md for more information.`);
		let toolPath = null;
	...
	async setDotnetRoot() {
		if (os.platform() !== "win32" && !this.buildAgent.getVariable("DOTNET_ROOT")) {
			...
			if ((await fs.lstat(dotnetPath)).isSymbolicLink()) dotnetPath = await fs.readlink(dotnetPath) || dotnetPath;
	...
	async executeTool(args) {
		...
		if (!toolPath) toolPath = await this.buildAgent.which(this.toolName, true);
	...
	async queryLatestMatch(toolName, versionSpec, includePrerelease) {
		...
		const version = import_semver.maxSatisfying(versions, versionSpec, { includePrerelease });
		if (version) this.buildAgent.info(`Found matching version: ${version}`);
		else this.buildAgent.info("match not found");
		return version;
	}
	async installTool(toolName, version, ignoreFailedSources, nugetConfigPath) {
		const semverVersion = import_semver.clean(version);
		if (!semverVersion) throw new Error(`Invalid version spec: ${version}`);
		const tempDirectory = await this.createTempDirectory();
		if (!tempDirectory) throw new Error("Unable to create temp directory");
		...
		if (ignoreFailedSources) builder.addFlag("ignore-failed-sources");
		if (nugetConfigPath) builder.addKeyValue("configfile", nugetConfigPath);
	...
	async createTempDirectory() {
		...
		if (!tempRootDir) throw new Error("Temp directory not set");
	...
	isExplicitVersion(versionSpec) {
		const cleanedVersionSpec = import_semver.clean(versionSpec);
		const valid = import_semver.valid(cleanedVersionSpec) != null;
...
var SettingsProvider = class {
	...
	getSetupSettings() {
		return {
			versionSpec: this.buildAgent.getInput("versionSpec"),
			includePrerelease: this.buildAgent.getBooleanInput("includePrerelease"),
			ignoreFailedSources: this.buildAgent.getBooleanInput("ignoreFailedSources"),
			preferLatestVersion: this.buildAgent.getBooleanInput("preferLatestVersion"),
			nugetConfigPath: this.buildAgent.getInput("nugetConfigPath", false)
		};
	}
};
//#endregion
//#region src/tools/common/index.ts
...
//#endregion
export { ArgumentsBuilder as a, DotnetTool as i, keysOf as n, SettingsProvider as r, RunnerBase as t };
